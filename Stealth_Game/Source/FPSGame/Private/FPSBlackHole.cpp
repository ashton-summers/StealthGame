...
AFPSBlackHole::AFPSBlackHole()
{
	...
	OuterSphereComp->SetCollisionEnabled(ECollisionEnabled::NoCollision);
...
{
	// Check overlapping component before destroying.
	if (OtherActor)
	{
		OtherActor->Destroy();
	}
...
void AFPSBlackHole::Tick(float DeltaTime)
{
	...
	for (int32 i = 0; i < OverlappingComps.Num(); i++)
	{
		...
		// Overlapping comp must be simulating physics.
		if (PrimComp && PrimComp->IsSimulatingPhysics())
		{
			const float SphereRadius = OuterSphereComp->GetScaledSphereRadius();
			const float ForceStrength = -2000;

			// Apply our force to bring overlapping components inward.
